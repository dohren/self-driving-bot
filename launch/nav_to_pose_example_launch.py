...
def generate_launch_description():
    ...
    subprocess.run(['pkill', '-9', '-f', 'rviz'])
    subprocess.run(['pkill', '-9', '-f', 'gz sim'])
    # subprocess.run(['pkill', '-9', '-f', 'ros'])
    ...
    robot_sdf = os.path.join(self_driving_dir, 'urdf', 'ikea_table.urdf')
    ...
    spawn_robot_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(self_driving_dir, 'launch', 'spawn_tb4.launch.py')),
